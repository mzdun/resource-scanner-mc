...
from .project import (Arg, Decl, GithubInfo, Project, Version, VersionSuite,
                      getVersion, getVersionFilePath, setVersion,
                      setVersionSuite)
...
class TestGradle(unittest.TestCase):
    ...
    def test_touchDeclReprAndStr(self):
        ...
        self.assertEqual("Decl(name='name', value='value', offset=123)", repr(decl))
    ...
    def test_getVersion(self):
        mock1 = cast(
            MagicMock,
            mock_open(
                read_data="""mod_version = 1.99.5061-alpha.5
...
"""
        ...
        self.assertEqual(
            project.version, Version(Arg("1.99.5061", 14), Arg("-alpha.5", 23))
        )
        self.assertEqual(project.github, GithubInfo("OWNER", "PROJECT"))
        self.assertEqual(project.github.url, "https://github.com/OWNER/PROJECT")
        ...
        self.assertEqual(
            repr(project),
            "GradleProject(packageRoot='ARCHIVE', version=Version(core=Arg(value='1.99.5061', offset=14), stability=Arg(value='-alpha.5', offset=23)), github=GithubInfo(owner='OWNER', repo='PROJECT'), properties={'mod_version': '1.99.5061-alpha.5', 'archives_base_name': 'ARCHIVE', 'url': 'https://github.com/OWNER/PROJECT/PACKAGE'})",
        )
        self.assertEqual(repr(project), repr(secondProject))
        mock1.assert_called_once_with(
            os.path.join(PROJECT_ROOT, "gradle.properties"), "r", encoding="UTF-8"
        )
    ...
    def test_getVersion_stable(self):
        mock = cast(
            MagicMock,
            mock_open(
                read_data="""mod_version = 1.99.5061
archives_base_name = ARCHIVE
url = https://github.com/OWNER/PROJECT/PACKAGE"""
            ),
        )
        ...
        self.assertEqual(
            repr(project),
            "GradleProject(packageRoot='ARCHIVE', version=Version(core=Arg(value='1.99.5061', offset=14), stability=Arg(value='', offset=23)), github=GithubInfo(owner='OWNER', repo='PROJECT'), properties={'mod_version': '1.99.5061', 'archives_base_name': 'ARCHIVE', 'url': 'https://github.com/OWNER/PROJECT/PACKAGE'})",
        )
    ...
    def test_getVersionFilePath(self):
        ...
        setVersionSuite(suite)
        self.assertEqual(
            os.path.join(PROJECT_ROOT, "gradle.properties"), getVersionFilePath()
        )
...
"""
    ...
    def test_setVersion_clearStability(self):
        mock = cast(
            MagicMock,
            mock_open(
                read_data="""mod_version = 1.99.5061-rc.1
archives_base_name = ARCHIVE
url = https://github.com/OWNER/PROJECT/PACKAGE
"""
            ),
        ...
        mock.return_value.write.assert_has_calls(
            [
                call(
                    """mod_version = 1.101.0-rc.1
archives_base_name = ARCHIVE
url = https://github.com/OWNER/PROJECT/PACKAGE
"""
                ),
                call(
                    """mod_version = 1.99.5061
archives_base_name = ARCHIVE
url = https://github.com/OWNER/PROJECT/PACKAGE
"""
                ),
