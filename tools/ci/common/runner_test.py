...
def setupEnv(
    mockedCall: str = "subprocess.check_call",
    dryRun: bool = False,
    useColor: bool = False,
    debug: bool = False,
    silent: bool = False,
...
class TestRunner(unittest.TestCase):
    ...
    def test_dryRun_run(self):
        with setupEnv(
            mockedCall="subprocess.run", dryRun=True, useColor=True
        ) as mock, io.StringIO() as buf:
            with contextlib.redirect_stderr(buf):
                run("app", "arg1", "arg2", "arg 3", "--flag")
            mock.assert_called_once_with(
                ("app", "arg1", "arg2", "arg 3", "--flag"), shell=False
    ...
    def test_dryRun_noColor_checkedCapture(self):
        with setupEnv(
            mockedCall="subprocess.run", dryRun=True
        ) as mock, io.StringIO() as buf:
            with contextlib.redirect_stderr(buf):
    ...
    def test_secret_run_checkedCapture(self):
        with setupEnv(
            mockedCall="subprocess.run", useColor=True
        ) as mock, io.StringIO() as buf:
            Environment.SECRETS = ["arg 3"]
            with contextlib.redirect_stderr(buf):
                checked_capture("app", "arg1", "arg2", "arg 3", "--flag")
            mock.assert_called_once_with(
                ("app", "arg1", "arg2", "arg 3", "--flag"),
                shell=False,
                capture_output=True,
                check=True,
            )
