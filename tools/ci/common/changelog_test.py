...
from .changelog import ChangelogFileUpdate, GithubReleaseChangelog, format_commit_message, release_changelog, update_changelog
    ...
    def _getLog(self, takeAll: bool = False):
        log, _ = TestCommit.parse(
            [
                ...
                TestCommit(HASH_E,
                           'feat(engine): change something in the engine, I guess'),
                TestCommit(HASH_F, 'style(sass): update the theme'),
                TestCommit(
                    HASH_G, 'custom section(new scope): that one is strange\n\nFixes: #1456'),
                TestCommit(
                    HASH_H, 'fix(layout)!: break the getPos'),
                TestCommit(
                    HASH_I, 'ci(github actions)!: update ubuntu runners to ubuntu-latest'),
            ],
            takeAll=takeAll
        )
        return log
...
'''.strip(), 'draft': True, 'prerelease': True},
...
''')
    ...
    def test_addNextEntryToChangelog(self):
        openMock = cast(MagicMock, mock_open(
            read_data='# Changelog\n\n## Previous release'))
        runMock = MagicMock(return_value=asProc(returncode=1))
        log = self._getLog()
        with patch('builtins.open', openMock), patch('subprocess.run', runMock), patch('time.strftime', MagicMock(return_value='TODAY')):
            update_changelog(log, 'v5.0.0', 'v4.78.1258', '{GITHUB}')
