...
from .changelog import BREAKING_CHANGE, CommitLink, Level
from .git import (addFiles, annotatedTag, bumpVersion, commit, getLog, getTags,
                  setCommitSep)
from .project import Arg, Project, Version
from .runner import Environment
from .test_commits import COMMIT_SEP as MOCK_COMMIT_SEP
...
class TestGit(unittest.TestCase):
    ...
    def test_getLog_HEAD(self):
        ...
        mock = MagicMock(return_value=asProc(stdout=commit.toStdout().encode("UTF-8")))
        with patch("subprocess.run", mock):
            log, level = getLog(["tag1"], {}, False)
        mock.assert_called_once_with(
            ("git", "log", f"--format=%h %H%n%B%n{MOCK_COMMIT_SEP}", "tag1..HEAD"),
            shell=False,
            capture_output=True,
        )
    ...
    def test_getLog_range(self):
        ...
        mock.assert_called_once_with(
            ("git", "log", f"--format=%h %H%n%B%n{MOCK_COMMIT_SEP}", "tag1..tag2"),
            shell=False,
            capture_output=True,
        )
        self.assertEqual(Level.PATCH, level)
    ...
    def test_addFiles(self):
        ...
        mock.assert_called_once_with(("git", "add", "A", "B", "C"), shell=False)
...
class TestCase(NamedTuple):
    ...
    def attach(case):
        ...
        if case.testName is None and case.name is None:
            raise RuntimeError(f"Both name and testName cannot be None;\n{case=}")
        ...
        def test_impl(self: TestGit):
            log, level = TestCommit.parse([TestCommit(HASH, msg)], takeAll=case.takeAll)
            self.assertEqual(level, case.expectedLevel)
            if case.expectedSection is None:
                self.assertEqual(log, {})
            else:
                self.assertEqual(log, {case.expectedSection: [case.expectedLog]})

        testName = (
            f"test_{case.name}" if case.testName is None else f"test_{case.testName}"
        )
...
for test_case in [
    ...
    GetTagsCase(name="range", version="0.1.0", range=["v0.1.0", "v1.0.0-alpha"]),
    GetTagsCase(
        name="fromAlpha", version="1.0.0-alpha", range=["v1.0.0-alpha", "v1.0.0"]
    ),
    GetTagsCase(
        name="fromBeta", version="1.0.0-beta", range=["v1.0.0-alpha", "v1.0.0"]
    ),
]:
    test_case.attach()
