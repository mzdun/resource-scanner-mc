...
from ..common.test_subprocess import (Always, SubprocessMock, Times,
                                      asJsonBytes, asJsonProc, asProc)
from .api import API
...
class TestGithubApi(unittest.TestCase):
    ...
    def test_constructor(self):
        ...
        with mock.patch(), envDbg(True) as buf:
            with contextlib.redirect_stderr(buf):
                api = API.fromProject(Project(None, None, GithubInfo("OWNER", "REPO")))
            self.assertEqual("", buf.getvalue())
    ...
    def test_release(self):
        ...
        mock = SubprocessMock()
        mock.matching("git", times=Always()).gh("OWNER", "REPO").matching(
            "/releases",
            method="POST",
            stdout=asJsonBytes({"data": "value"}),
            times=Times(2),
        )
    ...
    def test_getUnpublishedReleases(self):
        api = TestGithubApi.createApi("OWNER", "REPO")
        mock = SubprocessMock()
        mock.gh("OWNER", "REPO").matching(
            "/releases",
            stdout=asJsonBytes(
                [
                    {"data": "value", "tag_name": "v5.0.0"},
                    {"data": "value", "tag_name": "v4.5.0"},
                ]
            ),
        )
        ...
        self.assertEqual({"data": "value", "tag_name": "v4.5.0"}, data)
    ...
    def test_firstFrom(self):
        ...
        with mock.patch(), dryRun(False):
            data = api.first_from("/parented", root_name="root", tag_name="v7.1.0")
    ...
    def test_uploadAssets(self):
        api = TestGithubApi.createApi("OWNER", "REPO")
        mock = SubprocessMock()
        mock.matching("gh", "release", "upload", times=Always(), returncode=0)
        ...
        mock.checkCallMagic.assert_has_calls(
            [
                call(
                    ("gh", "release", "upload", "v1.2.3", "a", "b", "c", "--clobber"),
                    shell=False,
                )
            ]
        )
