...
public record Settings(int echoesSize, int blockDistance, int blockRadius, int lifetime, Set<Id> interestingIds) {

    private static final Gson GSON = new Gson();

    private record JsonSettings(int echoesSize, int blockDistance, int blockRadius, @Nullable Double lifetime,
    ...
    String serialize() {
        ...
        return GSON.toJson(new JsonSettings(echoesSize, blockDistance, blockRadius, lifetimeInSec, ids));
    }
    ...
    static Settings deserialize(String content) {
        try {
            ...
            if (settings == null || settings.interestingIds == null || settings.echoesSize < 0
                    ...
                    || (settings.lifetime != null && settings.lifetime < 0)) {
                return null;
            }
            ...
            return new Settings(settings.echoesSize, settings.blockDistance, settings.blockRadius, lifetime, ids);
    ...
    public Settings withEchoesSize(int value) {
        return new Settings(value, blockDistance, blockRadius, lifetime, interestingIds);
    }

    public Settings withBlockDistance(int value) {
        return new Settings(echoesSize, value, blockRadius, lifetime, interestingIds);
    }

    public Settings withBlockRadius(int value) {
        return new Settings(echoesSize, blockDistance, value, lifetime, interestingIds);
    }

    public Settings withLifetime(int value) {
        return new Settings(echoesSize, blockDistance, blockRadius, value, interestingIds);
    }

    public Settings withIds(Set<Id> value) {
        return new Settings(echoesSize, blockDistance, blockRadius, lifetime, value);
    }
